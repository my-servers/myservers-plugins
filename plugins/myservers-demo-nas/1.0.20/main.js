...
function demoComponentCatalog(ctx, label) {
  ...
  return [
    ...
    { id: "component-icon", title: "图标", subtitle: "Icon", description: "展示系统符号图标。", component: iconComponent },
    ...
    components: [
      {
        ...
        card: {
          ...
          children: [
            ...
            { id: "plugin-intro-summary", text: { text: "这是一个完全由插件动态生成的示例页面，用于展示组件布局、原生半弹层和受控交互能力。", style: "PLUGIN_TEXT_STYLE_BODY" } },
            {
              id: "plugin-feature-grid",
              grid: {
                columns: 2,
                spacing: 8,
                appearance: { container: "PLUGIN_CONTAINER_STYLE_NONE" },
                children: [
                  { id: "feature-components", value: { title: "组件目录", subtitle: "31 种原生组件", value: { text: "完整" }, appearance: { accent: demoAccent(ctx), icon: "square.grid.2x2.fill" } } },
                  { id: "feature-sheet", value: { title: "半弹层", subtitle: "多档原生高度", value: { text: "支持" }, appearance: { accent: "PLUGIN_ACCENT_PURPLE", icon: "rectangle.bottomhalf.inset.filled" } } },
                  { id: "feature-layout", value: { title: "动态布局", subtitle: "插件返回页面结构", value: { text: "灵活" }, appearance: { accent: "PLUGIN_ACCENT_TEAL", icon: "rectangle.3.group.fill" } } },
                  { id: "feature-action", value: { title: "受控交互", subtitle: "安全 Action 能力", value: { text: "可用" }, appearance: { accent: "PLUGIN_ACCENT_GREEN", icon: "bolt.shield.fill" } } }
                ]
              }
            },
            {
              ...
              button: {
                title: "查看组件示例",
                subtitle: "进入插件详情，点击卡片查看组件与代码",
                appearance: { accent: demoAccent(ctx), icon: "arrow.right.circle.fill", variant: "PLUGIN_COMPONENT_VARIANT_FILLED" },
                onTap: { plugin: { actionId: "openDetail" } }
              }
            },
            { id: "plugin-intro-footnote", text: { text: "示例版本 1.0.20 · 页面内容由 main.js 实时渲染", style: "PLUGIN_TEXT_STYLE_CAPTION" } }
          ]
        }
      }
    ]
