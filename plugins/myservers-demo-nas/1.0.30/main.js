...
function demoComponentCatalog(ctx, label) {
  ...
  const stateCustomComponent = {
    ...
    stateBlock: {
      ...
      appearance: { accent: demoAccent(ctx), iconSource: { systemName: "server.rack", url: "https://myservers.plus/images/logo.png" } }
    }
  };
  ...
  const iconComponent = {
    id: "render-icon",
    icon: { appearance: { accent: demoAccent(ctx), size: "PLUGIN_COMPONENT_SIZE_LARGE", iconSource: { systemName: "shippingbox.fill", url: "https://myservers.plus/images/logo.png" } } }
  };
    ...
    components: [
      {
        ...
        stack: {
          ...
          spacing: 8,
          children: [
            { id: "plugin-intro-icon", icon: { appearance: { accent: "PLUGIN_ACCENT_TEAL", size: "PLUGIN_COMPONENT_SIZE_LARGE", iconSource: { systemName: "puzzlepiece.extension.fill" } } } },
            { id: "plugin-intro-title", text: { text: "MyServers 插件组件示例", style: "PLUGIN_TEXT_STYLE_TITLE" } },
            { id: "plugin-intro-badge", badge: { text: "32 个组件", appearance: { accent: "PLUGIN_ACCENT_PURPLE" } } }
          ]
        }
      },
      { id: "plugin-intro-summary", text: { text: "查看插件组件、原生半弹层和示例代码。", style: "PLUGIN_TEXT_STYLE_BODY" } },
      {
        ...
        button: {
          title: "查看示例",
          appearance: { accent: demoAccent(ctx), iconSource: { systemName: "arrow.right.circle.fill" }, variant: "PLUGIN_COMPONENT_VARIANT_FILLED" },
  ...
  const baseChildren = [
    {
      ...
      stack: {
        ...
        spacing: 8,
        children: [
          { id: "widget-icon", icon: { appearance: { accent: demoAccent(ctx), iconSource: { systemName: "shippingbox.fill" } } } },
