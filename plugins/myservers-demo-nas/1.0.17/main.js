...
function bottomSheetComponents(ctx) {
  const route = ctx.route || "sheet-demo";
  return [
    { id: "sheet-title", text: { text: "原生半弹层", style: "PLUGIN_TEXT_STYLE_TITLE" } },
    { id: "sheet-route", text: { text: "当前路由：" + route, style: "PLUGIN_TEXT_STYLE_CAPTION" } },
    {
      id: "sheet-height-list",
      list: {
        title: "切换高度组合",
        items: [
          {
            title: "紧凑高度",
            subtitle: "固定 280pt",
            onTap: { navigate: { surface: "sheet", route: "sheet-compact", detents: ["compact"], initialDetent: "compact" } }
          },
          {
            title: "中等 / 全屏",
            subtitle: "可手势拖动切换",
            onTap: { navigate: { surface: "sheet", route: "sheet-flexible", detents: ["medium", "large"], initialDetent: "medium" } }
          }
        ],
        appearance: { accent: demoAccent(ctx), variant: "PLUGIN_COMPONENT_VARIANT_TINTED" }
      }
    },
    { id: "sheet-refresh", button: { title: "刷新当前弹层", appearance: { accent: demoAccent(ctx), icon: "arrow.clockwise" }, onTap: { plugin: { actionId: "sheetRefresh" } } } }
  ];
}
...
function demoComponentShowcase(ctx, label) {
  ...
  return [
    ...
    componentExample("component-bottom-sheet", "Bottom Sheet", "从详情页打开原生半弹层，并配置可用高度与初始高度。", bottomSheetComponent),
