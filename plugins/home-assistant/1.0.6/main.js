...
function quickToggleDetailAction(ctx, entity) {
  var attributes = entity && entity.attributes ? entity.attributes : {};
  var domain = domainOf(entity);
  var hasAdditionalControls =
    (domain === "light" && attributes.brightness !== undefined) ||
    (domain === "fan" && attributes.percentage !== undefined);
  return hasAdditionalControls ? quickControlAction(ctx, entity) : null;
}
...
function entityDisplayAction(ctx, entity) {
  var domain = domainOf(entity);
  if (QUICK_CONTROL_DOMAINS[domain] && configEnabled(ctx, "allow_controls", false) && !isUnavailable(entity)) {
    return quickControlAction(ctx, entity);
  }
  return { navigate: { surface: "detail", route: "entity:" + encodeURIComponent(String(entity.entity_id || "")) } };
}
