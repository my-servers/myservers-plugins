...
function dnsRecordHasHostnameAnalytics(record) {
  var type = String(record && record.type || "").toUpperCase();
  var name = String(record && record.name || "");
  return !!(record && record.proxied) && (type === "A" || type === "AAAA" || type === "CNAME") && name.indexOf("*.") !== 0;
}
...
function dnsList(snapshot, limit, prefix) {
  ...
  for (var index = 0; index < records.length && index < limit; index += 1) {
    var record = records[index];
    var recordID = String(record.id || index);
    var accent = record.proxied ? "PLUGIN_ACCENT_ORANGE" : "PLUGIN_ACCENT_GRAY";
    var route = dnsRecordHasHostnameAnalytics(record)
      ? zoneRoute(zoneID, "hostname-record") + ":" + record.id
      : zoneRoute(zoneID, "dns-record") + ":" + record.id;
    cards.push({
      id: prefix + "-dns-card-" + recordID,
      card: {
        appearance: { accent: accent, variant: "PLUGIN_COMPONENT_VARIANT_TINTED" },
        onTap: { navigate: { surface: "detail", route: route } },
        children: [{
          id: prefix + "-dns-row-" + recordID,
          stack: {
            axis: "PLUGIN_LAYOUT_AXIS_HORIZONTAL",
            spacing: 10,
            appearance: { container: "PLUGIN_CONTAINER_STYLE_NONE" },
            children: [
              { id: prefix + "-dns-icon-" + recordID, icon: { appearance: { accent: accent, iconSource: { systemName: record.proxied ? "cloud.fill" : "network" } } } },
              { id: prefix + "-dns-copy-" + recordID, stack: { spacing: 2, appearance: { container: "PLUGIN_CONTAINER_STYLE_NONE" }, children: [
                { id: prefix + "-dns-name-" + recordID, text: { text: record.name, style: "PLUGIN_TEXT_STYLE_SUBTITLE" } },
                { id: prefix + "-dns-subtitle-" + recordID, text: { text: record.type + " · " + record.content, style: "PLUGIN_TEXT_STYLE_CAPTION", appearance: { accent: "PLUGIN_ACCENT_GRAY" } } }
              ] } },
              { id: prefix + "-dns-spacer-" + recordID, spacer: {} },
              { id: prefix + "-dns-status-" + recordID, badge: { text: record.proxied ? "已代理" : "仅 DNS", appearance: { accent: accent } } }
            ]
          }
        }]
      }
  ...
  return { id: prefix + "-dns-list", stack: { spacing: 10, appearance: { container: "PLUGIN_CONTAINER_STYLE_NONE" }, children: [
    { id: prefix + "-dns-title", text: { text: "DNS 记录", style: "PLUGIN_TEXT_STYLE_SUBTITLE" } },
    { id: prefix + "-dns-items", stack: { spacing: 8, appearance: { container: "PLUGIN_CONTAINER_STYLE_NONE" }, children: cards } }
  ] } };
}
...
function hostnameDetailComponents(ctx, snapshot, hostname, record) {
  var zone = snapshot.zone || {};
  var rangeKey = selectedAnalyticsRange(ctx, zone.id);
  var analytics;
  try {
  ...
  if (record) {
    components.push({
      id: "hostname-dns-record-entry",
      value: {
        title: "DNS 记录详情",
        subtitle: record.type + " · " + record.content,
        value: { text: isEnabled(ctx, "enable_dns_write", false) ? "查看与编辑" : "查看" },
        appearance: { accent: "PLUGIN_ACCENT_INDIGO", iconSource: { systemName: "globe" }, variant: "PLUGIN_COMPONENT_VARIANT_TINTED" },
        onTap: { navigate: { surface: "detail", route: zoneRoute(zone.id, "dns-record") + ":" + record.id } }
      }
    });
  }
  return components;
...
globalThis.detail = function(ctx) {
  ...
  if (route.kind === "zones") {
    return { title: "概览", components: zoneSelectionComponents(accountSnapshot) };
  }
  ...
  if (route.section === "hostname-record") {
    var hostnameRecord = findDNSRecord(snapshot, route.recordID);
    if (!hostnameRecord) return { title: "子域名流量", components: [errorState("没有找到所选 DNS 记录，它可能已被删除或更新")] };
    return { title: hostnameRecord.name, components: hostnameDetailComponents(ctx, snapshot, hostnameRecord.name, hostnameRecord) };
  }
