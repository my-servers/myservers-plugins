...
var HTTP_ANALYTICS_DAILY_QUERY = HTTP_ANALYTICS_QUERY.split("datetimeHour").join("datetimeDay");
...
function queryAnalytics(ctx, zoneID, rangeKey) {
  ...
  for (index = 0; index < requestGroups.length; index += 1) {
    ...
    result.points.push({
      label: range.daily
        ? dayLabel(requestGroup.dimensions && requestGroup.dimensions.datetimeDay)
...
function analyticsRangeMenu(snapshot, prefix) {
  ...
  return {
    ...
    actionMenu: {
      title: current.label,
      items: items,
      appearance: { accent: "PLUGIN_ACCENT_ORANGE", iconSource: { systemName: "calendar.badge.clock" }, variant: "PLUGIN_COMPONENT_VARIANT_TINTED" }
    }
  };
}
...
function analyticsComponents(snapshot, prefix) {
  ...
  var result = [analyticsRangeMenu(snapshot, prefix)];
  if (points.length > 0) {
    result.push({
