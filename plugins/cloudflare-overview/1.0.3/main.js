...
var ACCOUNT_CACHE_KEY = "cloudflare-overview-account";
...
function parseRoute(route) {
  ...
  if (parts[0] === "zone" && parts[1]) {
    return { kind: "zone", zoneID: parts[1], section: parts[2] || "overview" };
  }
  if (parts[0] === "tunnels") {
      ...
      children: [
        {
          ...
          value: {
            ...
            onTap: { navigate: { surface: "detail", route: zoneRoute(zoneID, "analytics") } }
          }
        },
        {
          id: prefix + "-bandwidth",
        ...
        },
        {
          ...
          value: {
            ...
            onTap: { navigate: { surface: "detail", route: zoneRoute(zoneID, "dns") } }
          }
        },
        {
          id: prefix + "-tunnels",
          value: {
            ...
            onTap: { navigate: { surface: "detail", route: zoneRoute(zoneID, "tunnels") } }
          }
        }
      ]
    }
...
function dashboardComponents(ctx, snapshot, prefix) {
  ...
  var components = [{
    ...
    stack: {
      ...
      children: [
        { id: prefix + "-icon", icon: { appearance: { accent: "PLUGIN_ACCENT_ORANGE", size: "PLUGIN_COMPONENT_SIZE_LARGE", iconSource: iconSource("cloud.fill") } } },
        { id: prefix + "-title", text: { text: "Cloudflare", style: "PLUGIN_TEXT_STYLE_TITLE" } },
        { id: prefix + "-status", badge: { text: safeNumber(snapshot.zoneCount) + " 个域名", appearance: { accent: "PLUGIN_ACCENT_ORANGE" } } }
      ]
    }
  }];
  components.push({ id: prefix + "-subtitle", text: { text: accountCount + " 个账户 · " + safeNumber(snapshot.activeCount) + " 个正常", style: "PLUGIN_TEXT_STYLE_CAPTION", appearance: { accent: "PLUGIN_ACCENT_GRAY" } } });
  components.push(accountSummaryGrid(snapshot, prefix));
  components.push({
    id: prefix + "-open-detail",
    button: {
      title: "选择域名",
      subtitle: "进入域名后查看流量、DNS 与 Tunnel",
      appearance: {
...
function zoneList(snapshot) {
  ...
  return { id: "detail-zone-list", list: { title: "选择域名", items: items, appearance: { accent: "PLUGIN_ACCENT_ORANGE", variant: "PLUGIN_COMPONENT_VARIANT_TINTED" } } };
}
  ...
  var components = [
    {
      ...
      descriptionList: {
        ...
        columns: 2,
        items: [
          { title: "域名", value: { number: safeNumber(snapshot.zoneCount), format: "PLUGIN_VALUE_FORMAT_NUMBER" } },
          { title: "正常", value: { number: safeNumber(snapshot.activeCount), format: "PLUGIN_VALUE_FORMAT_NUMBER", status: "PLUGIN_STATUS_HEALTHY" } },
          { title: "账户", value: { number: accountCount, format: "PLUGIN_VALUE_FORMAT_NUMBER" } },
          { title: "已暂停", value: { number: safeNumber(snapshot.pausedCount), format: "PLUGIN_VALUE_FORMAT_NUMBER", status: safeNumber(snapshot.pausedCount) > 0 ? "PLUGIN_STATUS_WARNING" : "PLUGIN_STATUS_NEUTRAL" } }
        ],
        appearance: { accent: "PLUGIN_ACCENT_ORANGE" }
      }
    },
