  ...
  return [
    {
      ...
      stack: {
        ...
        children: [
          ...
          { id: prefix + "-status", badge: { text: zone.paused ? "已暂停" : statusText(zone.status), value: { status: zone.paused ? "PLUGIN_STATUS_WARNING" : stateStatus(zone.status) }, appearance: { accent: zone.paused ? "PLUGIN_ACCENT_ORANGE" : stateAccent(zone.status) } } }
        ]
      }
    },
...
function dashboardComponents(ctx, snapshot, prefix) {
  var components = headerComponents(snapshot, prefix);
  components.push(overviewGrid(snapshot, prefix));
  var analytics = analyticsComponents(snapshot, prefix);
  for (var index = 0; index < analytics.length; index += 1) components.push(analytics[index]);
  components.push(dnsList(snapshot, 5, prefix));
  if (isEnabled(ctx, "show_tunnels", true)) components.push(tunnelList(snapshot, 4, prefix));
  var warning = warningComponent(snapshot, prefix);
...
globalThis.dashboard = function(ctx) {
  ...
  return { title: "Cloudflare", components: dashboardComponents(ctx, snapshot, "dashboard") };
};
...
globalThis.detail = function(ctx) {
  ...
  components.push(zoneList(snapshot));
  components.push(overviewGrid(snapshot, "detail"));
  return { title: snapshot.zone.name || "Cloudflare", components: components };
};
