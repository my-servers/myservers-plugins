...
function queryHostnameSummaries(ctx, zoneID, rangeKey) {
  ...
  var cacheKey = HOSTNAME_CACHE_PREFIX + "summary-" + String(zoneID || "") + "-" + dates.range.key;
  ...
  var response = apiRequest(ctx, "/graphql", "POST", {
    ...
    variables: {
      zoneTag: zoneID,
      since: dates.since.toISOString(),
      until: dates.until.toISOString()
    }
  });
  if (response.errors && response.errors.length > 0) {
  ...
  var zones = response.data && response.data.viewer && response.data.viewer.zones;
  if (!zones || zones.length === 0) throw new Error("Analytics 没有返回当前 Zone 的子域名数据");
  ...
  for (var index = 0; index < groups.length; index += 1) {
    ...
    items.push({
      hostname: hostname,
      requests: safeNumber(group.count),
      bytes: safeNumber(group.sum && group.sum.edgeResponseBytes),
      visits: safeNumber(group.sum && group.sum.visits)
    });
  }
  items.sort(function(left, right) { return right.requests - left.requests; });
  var result = { fetchedAt: Date.now(), rangeKey: dates.range.key, rangeLabel: dates.range.label, items: items };
...
function queryHostnameAnalytics(ctx, zoneID, hostname, rangeKey) {
  var dates = analyticsRangeDates(rangeKey);
  var cacheKey = HOSTNAME_CACHE_PREFIX + "detail-" + String(zoneID || "") + "-" + dates.range.key + "-" + String(hostname || "");
  var cached = cachedSnapshot(ctx, cacheKey);
  if (cached && safeNumber(cached.fetchedAt) > Date.now() - SNAPSHOT_FRESH_MS) return cached;
  var response = apiRequest(ctx, "/graphql", "POST", {
  ...
  var zones = response.data && response.data.viewer && response.data.viewer.zones;
  if (!zones || zones.length === 0) throw new Error("Analytics 没有返回当前子域名的数据");
  var requestGroups = zones[0].requests || [];
  var cacheGroups = zones[0].cache || [];
  var result = {
    fetchedAt: Date.now(),
    hostname: hostname,
    rangeKey: dates.range.key,
    rangeLabel: dates.range.label,
    requests: 0,
    bytes: 0,
    visits: 0,
    cachedBytes: 0,
