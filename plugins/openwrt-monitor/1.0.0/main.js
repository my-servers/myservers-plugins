...
function dashboardPage(ctx, snapshot) {
  ...
  return {
    ...
    components: [
      {
        id: "openwrt-dashboard-card",
        card: {
          appearance: { accent: "PLUGIN_ACCENT_BLUE", variant: "PLUGIN_COMPONENT_VARIANT_TINTED" },
          children: [
            {
              id: "openwrt-dashboard-header",
              stack: {
                axis: "PLUGIN_LAYOUT_AXIS_HORIZONTAL",
                spacing: 10,
                children: [
                  { id: "openwrt-dashboard-icon", icon: { appearance: appearance("PLUGIN_ACCENT_BLUE", "wifi.router.fill", "PLUGIN_COMPONENT_VARIANT_FILLED") } },
                  { id: "openwrt-dashboard-title", text: { text: hostname, style: "PLUGIN_TEXT_STYLE_TITLE" } },
                  { id: "openwrt-dashboard-status", badge: { text: isOnline ? "在线" : "WAN 离线", appearance: appearance(isOnline ? "PLUGIN_ACCENT_GREEN" : "PLUGIN_ACCENT_RED", isOnline ? "checkmark.circle.fill" : "exclamationmark.circle.fill") } }
                ]
              }
            },
            { id: "openwrt-dashboard-release", text: { text: releaseText(board), style: "PLUGIN_TEXT_STYLE_CAPTION" } },
            {
              id: "openwrt-dashboard-metrics",
              grid: {
                columns: 3,
                spacing: 8,
                appearance: { container: "PLUGIN_CONTAINER_STYLE_NONE" },
                children: [
                  valueComponent("openwrt-dashboard-uptime", "运行", "持续运行时间", safeNumber(snapshot.info.uptime), "PLUGIN_VALUE_FORMAT_DURATION", "PLUGIN_ACCENT_GREEN", "clock.arrow.circlepath"),
                  valueComponent("openwrt-dashboard-memory", "内存", formatBytes(memory.used), memory.fraction, "PLUGIN_VALUE_FORMAT_PERCENT", "PLUGIN_ACCENT_PURPLE", "memorychip"),
                  textValueComponent("openwrt-dashboard-wan", "WAN", String(wan.proto || "-").toUpperCase(), interfaceAddress(wan), isOnline ? "PLUGIN_ACCENT_BLUE" : "PLUGIN_ACCENT_RED", "network", isOnline ? "PLUGIN_STATUS_HEALTHY" : "PLUGIN_STATUS_STOPPED")
                ]
              }
            }
          ],
...
function interfaceItems(interfaces) {
  var result = [];
...
function overviewPage(ctx, snapshot) {
  ...
  var components = [
    ...
    },
    {
      ...
      grid: {
        ...
        children: [
          ...
          valueComponent("openwrt-rx", "实时下载", snapshot.traffic.name, history.length ? history[history.length - 1].rxRate : 0, "PLUGIN_VALUE_FORMAT_BYTES", "PLUGIN_ACCENT_BLUE", "arrow.down.circle.fill"),
          valueComponent("openwrt-tx", "实时上传", snapshot.traffic.name, history.length ? history[history.length - 1].txRate : 0, "PLUGIN_VALUE_FORMAT_BYTES", "PLUGIN_ACCENT_ORANGE", "arrow.up.circle.fill")
        ]
      }
    },
    {
      ...
      multiLineChart: {
        title: "WAN 实时流量（B/s）",
        series: [
          { id: "download", name: "下载", points: historyPoints(history, "rxRate"), appearance: { accent: "PLUGIN_ACCENT_BLUE" } },
          { id: "upload", name: "上传", points: historyPoints(history, "txRate"), appearance: { accent: "PLUGIN_ACCENT_ORANGE" } }
        ],
    ...
    },
    {
      ...
      list: {
        title: "网络接口",
        items: interfaceItems(snapshot.interfaces),
        ...
        children: [
          {
            ...
            confirm: {
              ...
              cancelTitle: "取消",
              destructive: false,
              confirmAction: { plugin: { actionId: "reloadNetwork" } },
          ...
          },
          {
            ...
            confirm: {
              ...
              cancelTitle: "取消",
              destructive: true,
              confirmAction: { plugin: { actionId: "reboot" } },
...
function clientsPage(ctx) {
  ...
  for (index = 0; index < leases.length; index += 1) {
    ...
    items.push({
      title: String(lease.hostname || lease.macaddr || lease.mac || "未知设备"),
      subtitle: String(lease.ipaddr || lease.ip || "-") + " · " + String(lease.macaddr || lease.mac || "-").toUpperCase(),
      value: { text: lease.expires ? formatDuration(lease.expires) : "在线", status: "PLUGIN_STATUS_HEALTHY" },
      appearance: appearance("PLUGIN_ACCENT_GREEN", "desktopcomputer")
    });
  }
  if (items.length === 0 && hintsPayload && typeof hintsPayload === "object") {
    var keys = Object.keys(hintsPayload);
    for (index = 0; index < keys.length; index += 1) {
      var hint = hintsPayload[keys[index]] || {};
      items.push({
        title: String(hint.name || keys[index]),
        subtitle: String((hint.ipaddrs && hint.ipaddrs[0]) || (hint.ipv4 && hint.ipv4[0]) || "已发现主机"),
        value: { text: "已发现", status: "PLUGIN_STATUS_NEUTRAL" },
        appearance: appearance("PLUGIN_ACCENT_BLUE", "desktopcomputer")
      });
    }
  }
  if (items.length === 0) {
...
globalThis.detail = function (ctx) {
  try {
    ...
    return overviewPage(ctx, loadSnapshot(ctx, false));
  } catch (error) {
    return errorPage(error);
  }
};
...
globalThis.widget = function (ctx) {
  try {
    ...
    if (ctx.widgetSize === "large") {
      components.push({ id: "openwrt-widget-interfaces", list: { title: "接口", items: interfaceItems(snapshot.interfaces).slice(0, 3), appearance: { accent: "PLUGIN_ACCENT_BLUE" } } });
    }
