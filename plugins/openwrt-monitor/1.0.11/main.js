var OPENWRT_LOGO_URL = "https://avatars.githubusercontent.com/u/2528830?v=4";
...
var OPENWRT_INTERFACE_HISTORY_KEY = "openwrt-interface-traffic-history-v1";
var OPENWRT_CPU_SAMPLE_KEY = "openwrt-cpu-sample-v1";
...
function parseCPUStat(data) {
  ...
  if (!/^cpu\s/.test(firstLine)) return null;
  var fields = firstLine.split(/\s+/).slice(1, 9);
  if (fields.length < 4) return null;
  var total = 0;
  for (var index = 0; index < fields.length; index += 1) total += safeNumber(fields[index]);
  var idle = safeNumber(fields[3]) + safeNumber(fields[4]);
  return total > 0 ? { total: total, idle: idle } : null;
}

function cpuUsage(ctx) {
  var response = optionalCall(ctx, "file", "read", { path: "/proc/stat" }, {});
  var current = parseCPUStat(response.data);
  if (!current) return { available: false, fraction: 0, label: "不可用" };

  var previous = ctx.cache ? ctx.cache.get(OPENWRT_CPU_SAMPLE_KEY) : null;
  if (ctx.cache) ctx.cache.set(OPENWRT_CPU_SAMPLE_KEY, current, { ttlMs: HISTORY_TTL_MS });

  if (previous && current.total > safeNumber(previous.total)) {
    var totalDelta = current.total - safeNumber(previous.total);
    var idleDelta = Math.max(0, current.idle - safeNumber(previous.idle));
    return {
      available: true,
      fraction: clamp((totalDelta - idleDelta) / totalDelta, 0, 1),
      label: "当前"
    };
  }
  ...
  return {
    available: true,
    fraction: clamp((current.total - current.idle) / current.total, 0, 1),
    label: "开机平均"
  };
}

function deviceStatisticsByName(ctx, name) {
  name = String(name || "");
...
function deviceStatistics(ctx, wan) {
  return deviceStatisticsByName(ctx, String((wan && (wan.l3_device || wan.device)) || ""));
}

function appendTrafficPoint(history, rx, tx, now) {
  ...
  return history.length > 48 ? history.slice(history.length - 48) : history;
}

function updateHistory(ctx, rx, tx, now) {
  if (!ctx.cache) return [];
  var history = ctx.cache.get(OPENWRT_HISTORY_KEY) || [];
  history = appendTrafficPoint(history, rx, tx, now);
  ctx.cache.set(OPENWRT_HISTORY_KEY, history, { ttlMs: HISTORY_TTL_MS });
  return history;
}

function interfaceHistoryKey(item, index) {
  return String((item && (item.interface || item.l3_device || item.device)) || ("interface-" + index));
}

function updateInterfaceHistories(ctx, interfaces, now) {
  if (!ctx.cache) return {};
  var stored = ctx.cache.get(OPENWRT_INTERFACE_HISTORY_KEY) || {};
  if (!stored || typeof stored !== "object" || Array.isArray(stored)) stored = {};
  var histories = {};
  for (var index = 0; index < interfaces.length; index += 1) {
    var item = interfaces[index] || {};
    var key = interfaceHistoryKey(item, index);
    var device = String(item.l3_device || item.device || item.interface || "");
    var statistics = deviceStatisticsByName(ctx, device);
    histories[key] = appendTrafficPoint(stored[key] || [], statistics.rx, statistics.tx, now);
  }
  ctx.cache.set(OPENWRT_INTERFACE_HISTORY_KEY, histories, { ttlMs: HISTORY_TTL_MS });
  return histories;
}
...
function loadSnapshot(ctx, force) {
  ...
  var cpu = cpuUsage(ctx);
  var now = Date.now();
  var history = updateHistory(ctx, traffic.rx, traffic.tx, now);
  var interfaceHistories = updateInterfaceHistories(ctx, interfaces, now);
  var snapshot = {
    capturedAt: now,
    board: board,
    info: info,
    ...
    cpu: cpu,
    history: history,
    interfaceHistories: interfaceHistories
  };
...
function appearance(accent, icon, variant, iconURL) {
  return {
    ...
    iconSource: { systemName: icon || "wifi.router.fill", url: iconURL || "" }
  };
}
...
function valueComponent(id, title, subtitle, number, format, accent, icon, status, iconURL) {
  return {
    id: id,
    value: {
      title: title,
      ...
      appearance: appearance(accent, icon, undefined, iconURL)
    }
  };
}
...
function gaugeComponent(id, title, label, fraction, accent, icon, available) {
  return {
    id: id,
    chart: {
      title: title,
      kind: "PLUGIN_CHART_KIND_GAUGE",
      points: available === false ? [] : [{ label: label || "使用率", value: clamp(fraction, 0, 1) * 100 }],
      options: { min: 0, max: 100, hideRange: true, emptyText: "当前固件未开放 CPU 统计" },
      appearance: appearance(accent, icon)
    }
  };
}

function usageAccent(fraction, normalAccent) {
  if (fraction >= 0.85) return "PLUGIN_ACCENT_RED";
  if (fraction >= 0.65) return "PLUGIN_ACCENT_ORANGE";
  return normalAccent;
}
    ...
    components: [
      {
        ...
        value: {
          ...
          appearance: {
            ...
            iconSource: { systemName: "wifi.router.fill", url: OPENWRT_LOGO_URL },
            hideBackground: true
          },
...
function interfacePage(ctx, snapshot, index) {
  ...
  var historyKey = interfaceHistoryKey(item, index);
  var history = snapshot.interfaceHistories && snapshot.interfaceHistories[historyKey] ? snapshot.interfaceHistories[historyKey] : [];
  var latest = history.length ? history[history.length - 1] : { rxRate: 0, txRate: 0 };
  var components = [
    {
    ...
    {
      id: "openwrt-interface-rate-grid",
      grid: {
        columns: 2,
        spacing: 8,
        appearance: { container: "PLUGIN_CONTAINER_STYLE_NONE" },
        children: [
          rateValueComponent("openwrt-interface-rx", "实时下载", device, latest.rxRate, "PLUGIN_ACCENT_BLUE", "arrow.down.circle.fill"),
          rateValueComponent("openwrt-interface-tx", "实时上传", device, latest.txRate, "PLUGIN_ACCENT_ORANGE", "arrow.up.circle.fill")
        ]
      }
    },
    {
      id: "openwrt-interface-traffic-chart",
      multiLineChart: {
        title: "接口实时流量（KB/s）",
        series: [
        ...
        options: { showLabels: true, emptyText: "等待采集接口流量数据" },
...
function overviewPage(ctx, snapshot) {
  ...
  var wan = snapshot.wan || {};
  var cpu = snapshot.cpu || { available: false, fraction: 0, label: "不可用" };
  var load = loadAverage(info);
  var history = snapshot.history || [];
  var resourceChildren = cpu.available
    ? [
        gaugeComponent("openwrt-cpu-gauge", "CPU 使用率", cpu.label, cpu.fraction, usageAccent(cpu.fraction, "PLUGIN_ACCENT_GREEN"), "cpu", true),
        gaugeComponent("openwrt-memory-gauge", "内存使用率", "已用 " + formatBytes(memory.used), memory.fraction, usageAccent(memory.fraction, "PLUGIN_ACCENT_PURPLE"), "memorychip", memory.total > 0)
      ]
    : [
        ...
        valueComponent("openwrt-memory", "内存使用", formatBytes(memory.used) + " / " + formatBytes(memory.total), memory.fraction, "PLUGIN_VALUE_FORMAT_PERCENT", usageAccent(memory.fraction, "PLUGIN_ACCENT_PURPLE"), "memorychip")
      ];
  var components = [
    {
      ...
      table: {
        ...
        columns: [
          { key: "name", title: "项目", width: "PLUGIN_COMPONENT_SIZE_COMPACT" },
          { key: "value", title: "详情", width: "PLUGIN_COMPONENT_SIZE_LARGE" }
        ],
        rows: [
          { id: "model", values: { name: { text: "型号" }, value: { text: String(board.model || "未知") } } },
          { id: "firmware", values: { name: { text: "固件" }, value: { text: releaseText(board) } } },
          { id: "kernel", values: { name: { text: "内核" }, value: { text: String(board.kernel || "-") } } },
          { id: "uptime", values: { name: { text: "运行时间" }, value: { text: formatDuration(info.uptime) } } }
        ],
        appearance: { accent: "PLUGIN_ACCENT_BLUE", variant: "PLUGIN_COMPONENT_VARIANT_TINTED" }
      }
    },
    {
      id: "openwrt-resource-gauges",
      grid: {
        columns: 2,
        spacing: 8,
        appearance: { container: "PLUGIN_CONTAINER_STYLE_NONE" },
  ...
  return {
    title: String(board.hostname || "OpenWrt") + " · 概览",
    navigationActions: openWrtNavigationActions(ctx),
    components: components
  };
}

function openWrtNavigationActions(ctx) {
  var actions = [
    {
      title: "刷新状态",
      appearance: appearance("PLUGIN_ACCENT_BLUE", "arrow.clockwise"),
      onTap: { plugin: { actionId: "refresh" } }
    }
  ];
  if (!configEnabled(ctx, "allow_actions", false)) return actions;
  actions.push({
    title: "重载网络",
    subtitle: "网络连接可能会短暂中断",
    appearance: appearance("PLUGIN_ACCENT_ORANGE", "arrow.triangle.2.circlepath"),
    onTap: {
      navigate: {
        surface: "sheet",
        route: "operation:reload-network",
        detents: ["compact", "medium"],
...
function operationSheetPage(operation) {
  var reboot = operation === "reboot";
  return {
    title: reboot ? "重启路由器" : "重载网络",
    components: [
      {
        id: reboot ? "openwrt-reboot" : "openwrt-reload-network",
        confirm: {
          title: reboot ? "重启 OpenWrt？" : "重载网络？",
          message: reboot ? "路由器和当前网络将在重启期间不可用。" : "网络连接可能会短暂中断。",
          confirmTitle: reboot ? "重启" : "重载",
          cancelTitle: "取消",
          destructive: reboot,
          onConfirm: { plugin: { actionId: reboot ? "reboot" : "reloadNetwork" } },
          appearance: appearance(reboot ? "PLUGIN_ACCENT_RED" : "PLUGIN_ACCENT_ORANGE", reboot ? "power" : "arrow.triangle.2.circlepath")
        }
      }
    ]
  };
...
function clientDetailPage(ctx, index) {
  ...
  var detailRows = [
    { id: "mac", values: { name: { text: "MAC" }, value: { text: record.mac || "-" } } },
    { id: "source", values: { name: { text: "来源" }, value: { text: record.source } } },
    { id: "ipv4", values: { name: { text: "IPv4" }, value: { text: record.ipv4.length ? record.ipv4.join("、") : "未分配" } } },
    { id: "ipv6", values: { name: { text: "IPv6" }, value: { text: record.ipv6.length ? record.ipv6.join("、") : "未分配" } } },
    { id: "lease", values: { name: { text: "租约" }, value: { text: leaseText, status: record.online ? "PLUGIN_STATUS_HEALTHY" : "PLUGIN_STATUS_NEUTRAL" } } },
    { id: "allocation", values: { name: { text: "分配方式" }, value: { text: record.staticLease ? "静态" : "动态" } } }
  ];
  if (record.duid) detailRows.push({ id: "duid", values: { name: { text: "DUID" }, value: { text: record.duid } } });
  var components = [
    {
...
globalThis.sheet = function (ctx) {
  ...
  if (route === "operation:reload-network") return operationSheetPage("reload-network");
  if (route === "operation:reboot") return operationSheetPage("reboot");
...
globalThis.widget = function (ctx) {
  try {
    ...
    var components = [
      valueComponent("openwrt-widget-status", String(snapshot.board.hostname || "OpenWrt"), releaseText(snapshot.board), snapshot.wan && snapshot.wan.up ? 1 : 0, "PLUGIN_VALUE_FORMAT_NUMBER", snapshot.wan && snapshot.wan.up ? "PLUGIN_ACCENT_GREEN" : "PLUGIN_ACCENT_RED", "wifi.router.fill", undefined, OPENWRT_LOGO_URL)
    ];
